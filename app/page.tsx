...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-6xl p-6 text-sm">
      <h1 className="text-2xl font-bold">Banco de Horas — Folha de Ponto</h1>
      <p className="mt-1 text-gray-500">
        Suba a foto da folha, revise os horários e baixe a planilha pronta.
      </p>

      <section className="mt-6 grid grid-cols-1 gap-4 rounded-lg border p-4 md:grid-cols-4">
        <label className="flex flex-col gap-1 md:col-span-2">
          <span className="font-medium">Funcionário</span>
          <input className="rounded border px-2 py-1" value={funcionario} list="lista-funcs"
        ...
        <label className="flex flex-col gap-1">
          ...
          <select className="rounded border px-2 py-1" value={mes}
        ...
        <label className="flex flex-col gap-1">
          ...
          <input type="number" className="rounded border px-2 py-1" value={ano}
            onChange={(e) => setAno(Number(e.target.value))} />
        </label>
        <label className="flex flex-col gap-1 md:col-span-2">
          <span className="font-medium">Foto da folha</span>
          <input type="file" accept="image/*" className="rounded border px-2 py-1"
            onChange={(e) => escolherArquivo(e.target.files?.[0] ?? null)} />
        </label>
        <label className="flex flex-col gap-1 md:col-span-2">
          <span className="font-medium">Feriados extras (opcional)</span>
          <input className="rounded border px-2 py-1" value={feriadosTxt}
      ...
      <section className="mt-4 flex flex-wrap items-center gap-3 rounded-lg border border-dashed p-4">
        <span className="font-medium">Gerar em lote:</span>
        <span className="text-gray-500">todas as planilhas de {MESES[mes]}/{ano} salvas no banco →</span>
      ...
      {dias && (
        <section className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="lg:sticky lg:top-4 lg:self-start">
            ...
            {imgUrl
              ? <img src={imgUrl} alt="folha" className="max-h-[80vh] w-full rounded border object-contain" />
              : <p className="text-gray-400">Sem imagem (preenchimento manual).</p>}
          </div>
          ...
          <div>
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {dias.map((d) => {
                    ...
                    return (
                      <tr key={d.dia} className={fds ? 'bg-gray-50' : ''}>
                        <td className="border px-1 py-0.5 text-center" title={DIAS_SEMANA[wd]}>
                          <b>{d.dia}</b>
                          <span className="ml-1 text-[10px] text-gray-400">{DIAS_SEMANA[wd].slice(0, 3)}</span>
                        </td>
                        {CAMPOS.map((campo) => {
                          const k = `${d.dia}:${campo}`;
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <button onClick={salvarSheets} disabled={carregando || nErros > 0}
                className="rounded border border-green-600 px-4 py-2 font-medium text-green-700 disabled:opacity-50"
                title={nErros > 0 ? 'Corrija os erros antes de salvar' : ''}>
                Salvar no banco (Sheets)
              </button>
            </div>
            {aviso && <p className="mt-2 rounded bg-green-50 p-2 text-green-700">{aviso}</p>}
          </div>
        </section>
      )}
    </main>
